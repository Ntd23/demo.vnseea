...
// Page chat pins retain their legacy per-account semantics.
if ($chat_type === 'page') {
    ...
    if (!empty($info)) {
        ...
        $mutation_succeeded = (bool)$db->insert(T_MUTE, array(
            'user_id' => $wo['user']['id'],
            'type' => 'page',
            'time' => time(),
            'pin' => $pin_action,
...
if ($pin_action === 'yes' && !empty($existing_pin)) {
    $response_data = array(
        ...
        'pinned_by_user_id' => (string)$existing_pin->pinned_by,
        'can_unpin' => VNSEEA_CanUnpinSharedMessage($existing_pin, $chat_type, $chat_id)
    );
    return;
}
...
$db->startTransaction();
try {
    $pin_id = $db->insert(T_MESSAGE_PINS, array(
        'message_id' => $message_id,
        'pinned_by' => $current_user_id,
        'pinned_at' => $pinned_at
    ));
    if (empty($pin_id)) {
        throw new Exception('could not create shared message pin');
    }
    ...
    if (!empty($concurrent_pin)) {
        $response_data = array(
            ...
            'message' => 'message already pinned',
            'idempotent_replay' => true,
            'pinned_by_user_id' => (string)$concurrent_pin->pinned_by,
            'can_unpin' => VNSEEA_CanUnpinSharedMessage($concurrent_pin, $chat_type, $chat_id)
        );
    } else {
        $error_code = 5;
        $error_message = 'could not update pinned message';
