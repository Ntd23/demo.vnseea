...
function Wo_RegisterFollow($following_id = 0, $followers_id = 0)
{
    ...
    foreach ($followers_id as $follower_id) {
        ...
        $query = mysqli_query($sqlConnect, " INSERT INTO " . T_FOLLOWERS . " (`following_id`,`follower_id`,`active`,`notify`) VALUES ({$following_id},{$follower_id},'{$active}','1')");
        if ($query) {
...
function Wo_AcceptFollowRequest($following_id = 0, $follower_id = 0)
{
    ...
    $query = mysqli_query($sqlConnect, "UPDATE " . T_FOLLOWERS . " SET `active` = '1', `notify` = '1' WHERE `following_id` = {$follower_id} AND `follower_id` = {$following_id} AND `active` = '0'");
    if ($wo['config']['connectivitySystem'] == 1) {
        $query_two = mysqli_query($sqlConnect, "INSERT INTO " . T_FOLLOWERS . " (`following_id`,`follower_id`,`active`,`notify`) VALUES ({$following_id},{$follower_id},'1','1') ");
    }
    if ($query) {
        $notification_data = array(
            'recipient_id' => $following_id,
...
function VNSEEA_PrepareGroupMessageMentions($group_id, $text, $requested_user_ids = array())
{
    ...
    // Keep old Web/PHTML clients compatible, but only accept usernames that
    // resolve to an active member of this group.
    ...
    $users = function_exists('VNSEEA_GetChatUsersBatch')
        ? VNSEEA_GetChatUsersBatch($requested_ids)
        : array();
    $mention_ids = array();
    foreach ($requested_ids as $user_id) {
    ...
    return array(
        'ok' => true,
        'text' => $text,
        'mention_ids' => array_values($mention_ids),
        'invalid_ids' => array(),
    );
}
...
function VNSEEA_AttachMessageMentions($message)
{
    ...
    $source_text = !empty($message['or_text'])
    ...
    $users = function_exists('VNSEEA_GetChatUsersBatch')
        ? VNSEEA_GetChatUsersBatch($mention_ids)
        : array();
    $message['mentions'] = array();
    foreach ($mention_ids as $user_id) {
        $user = !empty($users[$user_id])
            ? $users[$user_id]
            : (function_exists('VNSEEA_GetMessageContextUser')
                ? VNSEEA_GetMessageContextUser($user_id)
                : Wo_UserData($user_id));
