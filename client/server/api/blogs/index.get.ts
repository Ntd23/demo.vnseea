...
const mapArticle = (entity: BackendEntity, currentUserId: number, index: number): BlogListArticle | null => {
  ...
  return {
    ...
    authorGradient: fallbackGradients[id % fallbackGradients.length],
    ...
    href: url || appRoutes.readBlog(slug),
...
export default defineEventHandler(async (event) => {
  ...
  return (response.articles ?? [])
    ...
    .map((entity, index) => mapArticle(entity, currentUserId, index))
