...
import { getBackendCheckoutBalances, normalizeCheckoutSnapshot } from "./_shared"
...
export default defineEventHandler(async (event): Promise<CheckoutSnapshot> => {
  ...
  const [checkout, addresses, balances] = await Promise.all([
    ...
    getBackendCheckoutBalances(event),
  ])

  return normalizeCheckoutSnapshot(event, checkout, addresses, balances)
})
