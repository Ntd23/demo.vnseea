...
| `NAV-USER-003` | `[ ]` | Desktop `1440x900` va Mobile `390x844` | User thuong khong thay Admin area | Login tai khoan khong phai admin | Khong render muc admin. |
| `NAV-USER-004` | `[ ]` | Desktop `1440x900` va Mobile `390x844` | Bootstrap user loi | Tat backend hoac lam `/_api/auth/me` fail | Menu khong hien thi du lieu gia, khong hien admin sai quyen. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `NAV-PHP-001` | Postman/API, doi chieu user menu desktop/mobile | PHP | `GET` | `{{backend_url}}/api/v2/endpoints/get-current-user.php?session_id={{user_id}}` | Cookie `user_id` | PHP tra `user_data.name`, `user_data.avatar`, `user_data.admin`, `wallet`, `points`. |
| `NAV-NUXT-001` | Postman/API, doi chieu user menu desktop/mobile | Nuxt | `GET` | `{{nuxt_url}}/_api/auth/me` | Cookie `user_id` | Nuxt map thanh `name`, `avatarUrl`, `isAdmin`, `wallet`, `points`; khong con mock user. |
