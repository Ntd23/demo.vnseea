  ...
  <div class="scrollbar-hide flex h-full w-full flex-col overflow-y-auto rounded-[32px] border border-white/70 bg-white/86 px-5 py-6 shadow-[0_28px_70px_rgba(15,23,42,0.10)] backdrop-blur-sm sm:px-6">
    <div class="flex flex-col items-center border-b border-secondary-100/80 pb-8">
      <div class="group relative mb-4 cursor-pointer">
        ...
        />
        <span class="absolute bottom-0 right-0 h-6 w-6 rounded-full border-4 border-white shadow-sm" :class="contact?.isOnline ? 'bg-sky-500' : 'bg-secondary-300'" />
      </div>
      <h3 class="text-[15px] font-black uppercase tracking-[0.18em] leading-none text-[var(--text-primary)]">{{ contact?.name || $t('pages.messagesPage.contactName') }}</h3>
      <div class="mt-2 flex items-center gap-2">
        <span class="h-1.5 w-1.5 rounded-full" :class="contact?.isOnline ? 'bg-sky-500' : 'bg-secondary-300'" />
