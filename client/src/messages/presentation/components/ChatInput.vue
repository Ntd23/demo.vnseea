  ...
  <div class="relative z-10 shrink-0 border-t border-secondary-100/80 bg-white/70 px-4 py-4 backdrop-blur-sm sm:px-6">
    <div class="mx-auto flex max-w-[1080px] items-center gap-3 rounded-[26px] border border-white/80 bg-white p-2.5 shadow-[0_20px_45px_rgba(15,23,42,0.08)]">
      <div class="hidden shrink-0 items-center gap-2 rounded-[18px] bg-secondary-50/80 p-1.5 ring-1 ring-secondary-100 sm:flex">
        <UButton
          variant="soft"
          class="h-10 w-10 shrink-0 rounded-[14px] bg-white text-[var(--text-primary)] ring-1 ring-secondary-100 transition-all shadow-none hover:bg-primary-50 hover:text-secondary-900 justify-center"
          square
        >
        ...
        <UButton
          ...
          :key="icon"
          variant="soft"
          class="h-10 w-10 rounded-[14px] bg-transparent text-[var(--text-primary)] transition-all shadow-none hover:bg-white hover:text-secondary-900 justify-center"
          square
        >
      ...
      <div class="relative flex-1">
        <textarea
          ...
          v-model="modelValue"
          rows="1"
          class="no-scrollbar w-full max-h-[140px] rounded-[20px] bg-secondary-50/75 px-5 py-3 pr-14 text-sm font-medium text-[var(--text-primary)] outline-none ring-1 ring-secondary-100 transition-all duration-300 focus:bg-white focus:ring-2 focus:ring-primary-500 resize-none"
        ...
        <div class="absolute right-2 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center">
          <MessagesPresentationMessageEmojiPicker @select="onEmoji" />
        </div>
      </div>
...
import MessagesPresentationMessageEmojiPicker from "./MessageEmojiPicker.vue"
