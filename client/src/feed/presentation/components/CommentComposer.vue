    ...
    <div class="comment-composer__shell">
      <div class="comment-composer__field">
        <div class="comment-composer__main-row">
          <div class="comment-composer__input-wrap">
            <div
              ...
              :class="{
                'comment-composer__editor--highlighted': hasHighlightedMentions,
              }"
            >
              <div
                v-if="hasHighlightedMentions"
                class="comment-composer__highlight"
                aria-hidden="true"
              >
                <template
                  v-for="segment in highlightedMessageSegments"
                  :key="segment.key"
                >
                  <span
                    :class="{
                      'comment-composer__highlight-mention': segment.isMention,
                    }"
                    >{{ segment.text }}</span
                  >
                </template>
            ...
            <div
              ...
              class="comment-composer__inline-actions"
            >
              <button
              ...
              >
                ...
                <Icon
                  v-else
                  name="i-ph-paper-plane-tilt-fill"
                  class="h-4.5 w-4.5"
                />
              </button>
            </div>
        ...
        <div
          ...
          class="comment-composer__mention-popover"
        >
          ...
          <template v-else-if="mentionQuery.trim().length > 0">
            ...
            >
              ...
              <span class="comment-composer__mention-copy">
                <span class="comment-composer__mention-name">{{
                  user.name
                }}</span>
                <span class="comment-composer__mention-username"
                  >@{{ user.username }}</span
                >
              </span>
            </button>
          ...
          <div
            v-if="!mentionLoading && mentionQuery.trim().length === 0"
            class="comment-composer__mention-state"
          >
            {{ $t("feed.commentComposer.mentionTypeToSearch") }}
          </div>
          <div
            v-else-if="!mentionLoading && mentionSuggestions.length === 0"
            class="comment-composer__mention-state"
          >
        ...
        <div
          v-if="attachmentPreview || recording || recordingErrorMessage"
          class="comment-composer__preview"
        >
          <template v-if="recording">
            <div class="comment-composer__recording-status">
              ...
              <span class="comment-composer__preview-label">{{
                $t("feed.commentComposer.recording")
              }}</span>
              <span class="comment-composer__recording-time">{{
                recordingDurationLabel
              }}</span>
            </div>
            <div
              class="comment-composer__recording-progress"
              aria-hidden="true"
            >
          ...
          <template v-else-if="attachmentPreview?.type === 'audio'">
            <div class="comment-composer__audio-preview">
              ...
              <button
                ...
                type="button"
                :aria-label="
                  audioPlaying ? 'Stop voice preview' : 'Play voice preview'
                "
                @click="toggleAudioPreview"
              >
                <Icon
                  :name="audioPlaying ? 'i-ph-stop-fill' : 'i-ph-play-fill'"
                  class="h-4 w-4"
                />
              </button>
              ...
              <div class="comment-composer__audio-track">
                <div class="comment-composer__audio-meta">
                  <span class="comment-composer__preview-label">
                    {{
                      ...
                      $t("feed.commentComposer.tooltipVoice")
                    }}
                  </span>
                  <span class="comment-composer__recording-time">{{
                    audioProgressLabel
                ...
                <div
                  class="comment-composer__audio-progress"
                  aria-hidden="true"
                >
                  <span
                    class="comment-composer__audio-progress-bar"
            ...
            <div class="comment-composer__preview-meta">
              <span class="comment-composer__preview-label">
                {{
                  ...
                  attachmentPreview?.name ||
                  $t("feed.commentComposer.selectedAttachment")
                }}
              </span>
              <button
...
const audioProgressPercent = computed(() => {
  ...
  return Math.min(
    100,
    Math.max(0, (audioCurrentTime.value / audioDuration.value) * 100),
  );
});
...
function createAudioFileFromChunks() {
  ...
  return new File([blob], `comment-audio-${Date.now()}.wav`, {
    type: "audio/wav",
  });
}
...
async function toggleRecording() {
  ...
  try {
    if (!canRecordAudio.value) {
      recordingErrorMessage.value = t(
        "feed.commentComposer.microphoneUnavailable",
      );
      toast.add({
        color: "warning",
    ...
    mediaSource.value = audioContext.value.createMediaStreamSource(
      mediaStream.value,
    );
