...
function applyQrPayload() {
  localError.value = ""
  console.log('[applyQrPayload] qrPayload:', qrPayload.value)
  const parsed = parseWalletQrPayload(qrPayload.value)
  console.log('[applyQrPayload] parsed:', parsed)

  if (!parsed?.to) {
    localError.value = t("pages.walletPage.errorQrPayload") + " (Raw: " + qrPayload.value + ")"
    return
  }
