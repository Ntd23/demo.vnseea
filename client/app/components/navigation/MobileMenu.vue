  ...
  <Teleport to="body">
    ...
    >
      <div v-if="open" class="fixed inset-0 z-[60]">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-primary-950/60 backdrop-blur-md transition-opacity duration-500" @click="$emit('close')" />
        
        <!-- Drawer Content -->
        <div class="absolute right-0 top-0 bottom-0 w-[85vw] max-w-[400px] overflow-y-auto overflow-x-hidden bg-primary-950 text-white shadow-[-12px_0_40px_rgba(0,0,0,0.5)] flex flex-col no-scrollbar">
          <!-- Decor -->
          <div class="absolute top-0 left-0 w-full h-96 bg-gradient-to-b from-primary-600/10 to-transparent pointer-events-none" />
          <div class="absolute -top-24 -right-24 w-64 h-64 bg-primary-500/20 rounded-full blur-[100px] pointer-events-none" />
          ...
          <div class="sticky top-0 z-20 flex items-center justify-between px-6 py-5 border-b border-white/5 bg-primary-950/80 backdrop-blur-xl">
            ...
            <button
              class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-white/60 transition-all hover:bg-white/10 hover:text-white"
              type="button"
          ...
          <!-- Navigation Groups -->
          <div class="flex-1 px-4 pb-8 space-y-8">
            <!-- Main Nav -->
            <div>
            ...
            <div class="grid grid-cols-3 gap-2 pt-4 border-t border-white/5">
              <button
                ...
                :key="item.label"
                class="flex flex-col items-center gap-3 rounded-2xl bg-white/5 border border-white/5 p-4 transition-all hover:bg-white/10 group"
                type="button"
              >
                <Icon :name="item.icon.includes('duotone') ? item.icon : item.icon.replace('-fill', '-duotone')" class="h-5 w-5 text-white/40 group-hover:text-primary-400" />
