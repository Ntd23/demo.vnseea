  ...
  <section class="rounded-[24px] border border-[#dbe3f2] bg-white p-5 shadow-[0_12px_30px_rgba(15,35,110,0.06)]">
    <div class="flex items-start justify-between gap-3">
      <div>
        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-[#0000ff]/70">
          {{ compact ? $t('community.pageSettings.sidebar.tips') : $t('community.pageSettings.basics.fields.summary') }}
        </p>
        <h3 class="mt-2 text-[1.2rem] font-black tracking-[-0.04em] text-[#243b63]">
          {{ compact ? $t('community.pageSettings.sidebar.preview') : page.name }}
        </h3>
      </div>
    ...
    <div class="mt-4 grid gap-3 sm:grid-cols-2">
      <div class="rounded-[18px] bg-[#f8fbff] px-4 py-3">
        <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-[#0000ff]/65">{{ $t('community.pageSettings.basics.fields.category') }}</p>
      ...
      <div class="rounded-[18px] bg-[#f8fbff] px-4 py-3">
        ...
        <p class="mt-1 text-[12px] leading-5 text-slate-500">
          {{
            [
              showFollowerCount ? $t('community.pages.format.followers', { count: followerCountLabel }) : "",
              showLikeCount ? $t('community.pages.format.likes', { count: likeCountLabel }) : "",
            ].filter(Boolean).join(" · ") || $t('community.pageSettings.sidebar.hidden')
          }}
        </p>
      </div>
    </div>
    ...
    <div v-if="!compact" class="mt-5">
      <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-[#0000ff]/70">
        {{ $t('community.pageSettings.basics.fields.tags') }}
      </p>
