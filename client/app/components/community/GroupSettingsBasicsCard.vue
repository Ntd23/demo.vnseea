<template>
  <CommunitySettingsSectionCard
    eyebrow="Thông tin cốt lõi"
    title="Hồ sơ hiển thị của nhóm"
    description="Cập nhật tên, URL, mô tả và nội dung nền để trang nhóm rõ ràng hơn khi thành viên ghé thăm."
  ...
  >
    ...
    <div class="space-y-5">
      <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.82fr)]">
        <label class="block">
          <span class="text-[15px] font-black text-[#243b63]">Tên nhóm</span>
          <input
            v-model="model.name"
        ...
        <label class="block">
          <span class="text-[15px] font-black text-[#243b63]">URL nhóm</span>
      ...
      <label class="block">
        <span class="text-[15px] font-black text-[#243b63]">Mô tả nhóm</span>
        <textarea
          ...
          placeholder="Mô tả nhóm này dành cho ai, nội dung chính là gì và cách tham gia hiệu quả."
        />
      </label>

      <div class="grid gap-5 lg:grid-cols-2">
        <label class="block">
          <span class="text-[15px] font-black text-[#243b63]">Danh mục</span>
          <div class="relative mt-3">
            <select
        ...
        <label class="block">
          <span class="text-[15px] font-black text-[#243b63]">Khu vực hoạt động</span>
          <input
            v-model="model.locationLabel"
            class="mt-3 h-14 w-full rounded-[18px] border border-[#dbe3f2] bg-white px-4 text-[15px] text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#0000ff]/35 focus:ring-4 focus:ring-[#0000ff]/8"
            placeholder="Ví dụ: Hoạt động toàn quốc"
            type="text"
          >
        </label>
      </div>
      ...
      <div class="grid gap-5 lg:grid-cols-2">
        <label class="block">
          <span class="text-[15px] font-black text-[#243b63]">Website hoặc link giới thiệu</span>
          <input
            v-model="model.website"
            class="mt-3 h-14 w-full rounded-[18px] border border-[#dbe3f2] bg-white px-4 text-[15px] text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#0000ff]/35 focus:ring-4 focus:ring-[#0000ff]/8"
            placeholder="vnseea.vn/g/ten-nhom"
            type="text"
          >
        </label>

        <label class="block">
          <span class="text-[15px] font-black text-[#243b63]">Thẻ chủ đề</span>
          <input
            v-model="model.tags"
            class="mt-3 h-14 w-full rounded-[18px] border border-[#dbe3f2] bg-white px-4 text-[15px] text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#0000ff]/35 focus:ring-4 focus:ring-[#0000ff]/8"
            placeholder="xe điện, trạm sạc, review"
            type="text"
          >
        </label>
      </div>
      ...
      <label class="block">
        <span class="text-[15px] font-black text-[#243b63]">Nội quy hiển thị trên trang nhóm</span>
        <textarea
          ...
          placeholder="Mỗi dòng là một nguyên tắc. Ví dụ: Ưu tiên bài chia sẻ có ngữ cảnh thực tế."
        />
      </label>

      <div class="grid gap-3 sm:grid-cols-3">
        <div class="rounded-[18px] bg-[#f8fbff] px-4 py-3">
          <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-[#0000ff]/65">Độ dài mô tả</p>
          <p class="mt-1 text-[15px] font-black text-[#243b63]">{{ model.summary.trim().length }} ký tự</p>
        </div>
        <div class="rounded-[18px] bg-[#f8fbff] px-4 py-3">
          <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-[#0000ff]/65">Tag hiện có</p>
          <p class="mt-1 text-[15px] font-black text-[#243b63]">{{ tagCount }} chủ đề</p>
        </div>
        <div class="rounded-[18px] bg-[#f8fbff] px-4 py-3">
          <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-[#0000ff]/65">Nội quy</p>
          <p class="mt-1 text-[15px] font-black text-[#243b63]">{{ guidelineCount }} mục</p>
        </div>
      </div>
    </div>
  </CommunitySettingsSectionCard>
