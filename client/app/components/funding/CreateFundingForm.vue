<template>
  <form class="rounded-[30px] border border-[var(--border-default)] bg-white p-5 shadow-[var(--shadow-lg)]" @submit.prevent="submit">
    <p class="text-label-secondary text-[var(--color-primary-600)]">Tạo funding</p>
    <h1 class="mt-1 text-heading text-[var(--text-primary)]">Chiến dịch mới</h1>
    <p class="mt-1 text-body-secondary">Nhập tiêu đề, mô tả, mục tiêu và ảnh đại diện chiến dịch.</p>

    <div class="mt-5 grid gap-4 sm:grid-cols-2">
      <label class="block sm:col-span-2">
        <span class="text-[12px] font-bold text-[var(--text-secondary)]">Tiêu đề</span>
        <input v-model="form.title" required class="funding-input mt-2" placeholder="Tên chiến dịch gây quỹ">
      </label>
      <label class="block">
        <span class="text-[12px] font-bold text-[var(--text-secondary)]">Lĩnh vực</span>
      ...
      <label class="block">
        <span class="text-[12px] font-bold text-[var(--text-secondary)]">Mục tiêu</span>
        <input v-model.number="form.goalAmount" required class="funding-input mt-2" min="1000000" type="number">
      </label>
      <label class="block sm:col-span-2">
        <span class="text-[12px] font-bold text-[var(--text-secondary)]">Ảnh chiến dịch</span>
      ...
      <label class="block sm:col-span-2">
        ...
        <textarea v-model="form.description" required class="funding-input mt-2 min-h-[160px] resize-y py-3" placeholder="Mục tiêu, đối tượng nhận hỗ trợ, cách sử dụng nguồn quỹ." />
      </label>
    </div>
    ...
    <div v-if="created" class="mt-4 rounded-[18px] bg-[var(--color-primary-50)] px-4 py-3 text-[13px] font-bold text-[var(--color-primary-600)]">
      Đã mô phỏng tạo chiến dịch. Chưa gọi API funding.php.
    </div>

    <div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
      <NuxtLink to="/funding" class="inline-flex h-11 items-center justify-center rounded-[18px] border border-[var(--border-default)] bg-white px-4 text-[13px] font-bold text-[var(--text-secondary)]">Quay lại</NuxtLink>
      <button class="inline-flex h-11 items-center justify-center rounded-[18px] bg-[var(--color-primary-500)] px-5 text-[13px] font-extrabold text-white shadow-[var(--shadow-brand)]" type="submit">Tạo chiến dịch</button>
    </div>
  </form>
</template>
