  ...
  <aside class="space-y-4">
    <section class="rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-white p-4 shadow-[var(--shadow-md)]">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-label-secondary text-[var(--color-primary-600)]">Chủ chiến dịch</p>
    ...
    <section class="rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-white p-4 shadow-[var(--shadow-md)]">
      <p class="text-label-secondary text-[var(--color-primary-600)]">Donors</p>
      <div class="mt-4 space-y-3">
        <div v-for="donor in campaign.donors" :key="donor.id" class="flex gap-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="text-[12px] font-bold text-[var(--color-primary-600)]">{{ formatFundingCurrency(donor.amount) }}</p>
    ...
    <section v-if="campaign.isOwner" class="rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-white p-4 shadow-[var(--shadow-md)]">
      <p class="text-label-secondary text-[var(--color-primary-600)]">Quản lý</p>
      <div class="mt-4 grid gap-2">
        <button class="h-10 rounded-[16px] bg-[var(--color-primary-50)] text-[13px] font-bold text-[var(--color-primary-600)]" type="button" @click="ownerMessage = 'Đã mô phỏng chỉnh sửa chiến dịch. Chưa gọi API funding.php.'">Chỉnh sửa</button>
        <button class="h-10 rounded-[16px] bg-[var(--color-secondary-100)] text-[13px] font-bold text-[var(--text-secondary)]" type="button" @click="ownerMessage = 'Đã mô phỏng xoá chiến dịch. Chưa gọi API funding.php.'">Xóa</button>
      </div>
